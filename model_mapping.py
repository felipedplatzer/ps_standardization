...
def map_one_make(this_make_source_df, this_make_target_df, make_target):
    if make_target.strip() == '':
        output_df = this_make_source_df[['model_name_source','modality_source']]
...
def process_make(make_target, source_df, target_df, filepath_dict, i, target_modality_list):
    print(f"Processing manufacturer {str(i+1)} of {len(source_df['make_target'].unique())}: {make_target}")
...
if __name__ == "__main__":
    ...
    #x_0 = input("Enter first make index: ")
    #x_1 = input("Enter last make index: ")
    print(f"Found {len(x)} new (i.e. not yet mapped) makes")
        
    with ProcessPoolExecutor(max_workers=config.MAX_WORKERS) as executor:
        futures = [executor.submit(process_make, make_target, source_df, target_df, filepath_dict, i, target_modality_list) for i, make_target in enumerate(x)]
        n_completed = 0
        for future in as_completed(futures):
            n_completed += 1
            print(f"Completed {n_completed} of {len(x)} makes")

    """
    for i, make_target in enumerate(x):
        if i < int(x_0) or i > int(x_1):
            pass
        else:
            process_make(make_target, source_df, target_df, filepath_dict, i)
    """
    ...
    if len(batch_files) > 0:
        ...
        print(f'\nMake/model summary')
        df = config.save_new_file(df, filepath_dict['model_mapping'], append_to_old=True, timeout=config.CONCURRENT_WRITE_TIMEOUT_LONG, unique_cols=['make_target', 'model_name_source'], tiebreak_cols=['mel_id','modality_target'], print_stats=True) # df contains records from previous runs
        for file in batch_files:
            os.remove(file)
    else:
